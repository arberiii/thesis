...
\chapter{KP-labeling and induced subgraphs of Hamming Graphs}
\thispagestyle{fancy}
...
We first state two labeling conditions needed:
\newline
...
\begin{theorem}[Staton and Wingard 1998]
Every d-realizable graph is \{$K_{1,d+1}$ , diamond,
$K_{2,3}$ , $C_5$ \}-free.
\end{theorem} 
\begin{figure}[h!]
\begin{center}
\includegraphics[width=0.4\linewidth]{figures/k23notrealizable.png}
\end{center}
\caption{$K_{2,3}$ is not realizable}
\end{figure}
Next we see the relation between $KP$-labeling and Cartesian dimension:
 \begin{theorem}[Kla\v zvar and Peterin 2005]
