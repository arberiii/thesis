...
\end{lemma}
\begin{proof}
The labels of a $u, v$-path of length $2$ on a $C_k$ are different, hence by Condition 2 those two labels should appear also along the other path. So for any label take two vertices $u$ and $v$ that have distance two and such that a $u,v$-path of length two contains the edge with that label and therefore the label is present more than once on $C_k$ 
\end{proof}
