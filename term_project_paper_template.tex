...
Kraj:\vspace{0.5cm}\\
Leto:\vspace{0.5cm}\\
\v Stevilo listov: 37 \hspace{1.6cm} \v Stevilo slik: 14 \vspace{0.5cm}\\
%\v Stevilo prilog: \hspace{1.9cm}
%\v Stevilo strani prilog: \hspace{1cm}
\v Stevilo referenc: 26\vspace{0.5cm}\\
Mentor: prof.~dr.~Martin Milani\v c\vspace{0.5cm}\\
Somentor:  prof.~dr.~Marcelo Mydlarz\vspace{0.5cm}\\
Klju\v cne besede: graf, induciran podgraf, Hammingov graf, KP-ozna\v citev, kartezi\v cna dimenzija, Hammingova dimenzija
\vspace{0.5cm}\\
Math.~Subj.~Class.~(2010): 05C10, 05C15, 68R10\vspace{0.5cm}\\
{\bf Izvle\v cek:} V tej seminarski nalogi obravnavamo inducirane podgrafe Hammingovih grafov. Inducirane vlo\v zitve grafov v ve\v cje grafe s posebno strukturo pogosto omogo\v cajo bolj\v se razumevanje lastnosti danih grafov. Z uporabo vlo\v zitev v kartezi\v cne produkte kvocientnih grafov sta Klav\v zar in Peterin leta 2005 opisala inducirane podgrafe Hammingovih grafov. V seminarski nalogi preu\v cujemo kartezi\v cno dimenzijo grafa, invarianto, ki so jo leta 2017 vpeljali Milani\v c, Mur\v si\v c in Mydlarz.  \v Studija zajema tako strukturne kot ra\v cunske vidike kartezi\v cne dimenzije grafov. Na kratko si ogledamo tudi Hammingovo dimenzijo grafov, ki so jo leta 2013 vpeljali Klav\v zar, Peterin in Zemlji\v c.
...
%The \emph{adjacency matrix} $A(G)=(a_{ij})_{i,j}^n$ of a graph $G$ with vertex set $\{v_1,v_2,\ldots, v_n\}$ is a square symmetric (0,1)-matrix whose rows and columns correspond to the vertices of $G$ and $a_{ij} = 1$ if and only if vertices $v_i$ and $v_j$ are adjacent.
...
%
%\begin{proof}
%Suppose two maximal cliques $A_1$ and $A_2$ intersect in at least two vertices, say $v_1, v_2$. Let $v_3$ be a vertex in $A_1$ non-adjacent to some vertex $v_4\in A_2$. {\color{blue}Note that such a pair of vertices exists since otherwise $A_1\cup A_2$ would be a clique properly containing $A_1$, contradicting the fact that $A_1$ is a maximal clique.} But then vertices $v_1,v_2,v_3,v_4$ induce a diamond, a contradiction with $G$ being diamond-free.
%\end{proof}
...
%If $G$ is complete, then we know that $\Cdim(G)\le 1$. Suppose that $G$ is not complete.
...
% where $\varphi$ is the map from vertex set to $\mathbb{R}^d$.
...
%\} \cup \{(1,k)\mid k\geq 3\} \cup \{(k,1) \mid k\geq 3\}$;} \\
				3, & \text{if $i = 2$ and $j\ge 3$ or vice versa;}\\
%$(i,j)\in \{(2,k)\mid k \geq 3\} \cup \{(k,2)\mid  |k\geq 3\}$;} \\
				4, & \text{if $i\ge 3$ and $j\ge 3$;}
%$(i,j)\in \{(k,\ell)\mid k,\ell\geq 3\}$;}
			\end{array}
		\right.
\]
\end{theorem}
