...
An interesting class of graph is the Cartesian product of two paths. We will see that we can find the exact Cartesian dimension of those graphs. It will be interesting as we will see an application of relation between Cartesian dimension and KP-labeling. \newline
First we start by dealing with small length of paths. Without loss of generality let us take $i\leq j$.\newline
If $i=1$ then $P_1\square P_j=P_j$ then $Cdim(P_i\square P_j)=0$ if $j=1$, $Cdim(P_i\square P_j)=1$ if $j=2$ and $Cdim(P_i\square P_j)=2$ if $j\geq 2$.\newline
If $i=2$ then $Cdim(P_i\square P_j)=2$ if $j=2$ and we write as proposition when $j\geq 3$
\begin{proposition}
...
For any non-adjacent vertices $u$ and $v$ if they belong to $C_4$ take label 1 and the other one to satisfy the Condition 2 and otherwise take labels 2 and 3 as for each pair of vertices that do not belong to $C_4$ then they belong to different components if we remove the edges of label 2 or 3.\newline
...
And now we will look at general case when $i\geq 3$.
\begin{proposition}
...
Indeed this is a KP-labeling as for any non-adjacent vertices $u$ and $v$ if they belong to a $C_4$ then take the labels in this cycle, otherwise either horizontally or vertically $u$ and $v$ are distanced by more than length of 2(to go from $u$ to $v$ you have to make two hoops vertically or horizontally) so if it is horizontally take labels 2 and 3 and if vertically take labels 1 and 4 and similarly as in the previous proposition then vertices $u$ and $v$ would belong to different components when considering removing one of the labels we choose. \newline
